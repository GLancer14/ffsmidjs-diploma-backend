...
### Docker
Для запуска приложения с помощью Docker необходимо разместить репозитории с серверной частью приложения (данный репозиторий) и [клиентской частью приложения](https://github.com/GLancer14/ffsmidjs-diploma-frontend) в одном каталоге, переместить файл docker-compose.yml в этот каталог и создать в нём папку volume (для хранения файлов базы данных и изображний). Затем открыть терминал в данном каталоге и запустить команду: `docker compose up --build`.
